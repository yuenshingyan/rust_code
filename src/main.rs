...
async fn main() -> Result<(), reqwest::Error> {
    ...
    headers.insert("referer", "https://www.oddsportal.com/football/england/premier-league-2013-2014/results/".parse().unwrap());
    ...
    let response = client.get("https://www.oddsportal.com/ajax-sport-country-tournament-archive_/1/tdkpynmB/X0/1/0/page/2/")
    .headers(headers)
    .send()
    .await?
    .json::<Value>()
    ...
    let mut all_fields_res: Vec<Series> = Vec::with_capacity(3);
    let all_fields: [&str; 2] = ["home-name", "away-name"];
    for f in all_fields {
        let mut fields: Vec<String> = Vec::with_capacity(json_len);
        for i in 0..json_len {
    ...
    let mut all_home_odds: Vec<String> = Vec::with_capacity(json_len);
    let mut all_draw_odds: Vec<String> = Vec::with_capacity(json_len);
    let mut all_away_odds: Vec<String> = Vec::with_capacity(json_len);
    ...
    let mut df = DataFrame::new(all_fields_res).unwrap();

    df.with_column(
        df.column("home_odds").unwrap()
    ...
    //             .with_column(
    //                 (
    //                     col("home_odds").cast(DataType::Float64) 
    //                     - col("away_odds").cast(DataType::Float64)
    //                 ).alias("odd_spread")
    //             )
    //             .collect()
    //             .unwrap();
